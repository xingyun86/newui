...
public:
	...
	STDMETHODIMP OnStatusTextChange(IDispatch* pDisp, BSTR bstrText) {
		...
		SetDlgItemTextW(hWndParent, dwEditStatusID, bstrText);
		return S_OK;
	}
...
public:
	...
	~IexplorerUtils()
	{
		...
		spConnectionPointContainer.Release();
		spConnectionPointBrowserEvents.Release();
	}
public:
	HWND hWnd = NULL;
	DWORD dwCookie = 0;
	...
	CComPtr<IConnectionPoint> spConnectionPointBrowserEvents = NULL;
	CComPtr<WebBrowserSink> pWebBrowserSink = new WebBrowserSink(this);
	CComPtr<IConnectionPointContainer> spConnectionPointContainer = NULL;
...
private:
	...
	int SelectMail_old(const CComQIPtr<IHTMLDocument2>& spIHTMLDocument2)
	{
		CComPtr<IHTMLElement> pIHTMLElement = NULL;
		spIHTMLDocument2->elementFromPoint(310, 288, &pIHTMLElement);
		if (pIHTMLElement != NULL)
		{
			pIHTMLElement->click();
			return(0);
...
public:
	...
	void Nav2Url(const BSTR url)
	{
		...
		VariantInit(&flags);
		flags.vt = VT_I4;
		flags.lVal = navNoHistory | navNoReadFromCache | navNoWriteToCache;
		pIWebBrowser2->Navigate2(&varUrl, &flags, 0, 0, 0);
		VariantClear(&flags);
	...
	int LoadHtml(const std::string & strHtml)
	{
		int nResult = (-1);
		CComPtr<IDispatch> pIDispatch = NULL;
		if (SUCCEEDED(pIWebBrowser2->get_Document(&pIDispatch)) && (pIDispatch != NULL))
		{
			...
			if (!strHtml.empty())
			{
				pstrHtml = strHtml.data();
			}
			LPSTR lpMemory = (LPSTR)::GlobalAlloc(GPTR, (strHtml.length() + 1) * sizeof(CHAR));
			if (lpMemory != NULL)
			{
				lstrcpyA(lpMemory, strHtml.c_str());
...
public:
	...
	void RegisterIEEventDealer()
	{
		// 声明一个IConnectionPointContainer和IConnectionPoint实例。    
		// 利用 IWebBrowser2 接口的 QueryInterface 方法获得 IConnectionPointContainer 接口  
		if (SUCCEEDED(pIWebBrowser2->QueryInterface(IID_IConnectionPointContainer, (void**)&spConnectionPointContainer)))
		{
			// 利用 IConnectionPointContainer 接口的 FindConnectionPoint 获取 IID为DIID_DWebBrowserEvents2 的连接点  
			if (SUCCEEDED(spConnectionPointContainer->FindConnectionPoint(DIID_DWebBrowserEvents2, &spConnectionPointBrowserEvents)))
			{
				...
				if (SUCCEEDED(spConnectionPointBrowserEvents->Advise((IUnknown*)pWebBrowserSink, &dwCookie)))
				{
					// Successfully advised
				}
			}
	...
	{
		if (spConnectionPointBrowserEvents != NULL)
		{
			spConnectionPointBrowserEvents->Unadvise(dwCookie);
		}
	}
	HRESULT Init(HWND hWndParent, 
		RECT& rect,
	...
	)
	{
		...
		hWnd = axWindowContainer.Create(hWndParent, &rect, lpWindowName, dwStyle, dwExStyle, MenuOrID, lpCreateParam);
		if (hWnd != NULL)
		{
			this->hWndParent = hWndParent;
			hResult = axWindowContainer.CreateControl(OLE_IEXPLORER_2_NAME);
			if (SUCCEEDED(hResult))
			{
				hResult = axWindowContainer.QueryControl(__uuidof(IWebBrowser2), (void**)&pIWebBrowser2);
				SetBrowserFeatureControl();
				RegisterIEEventDealer();
				Nav2Url(OLESTR("about:blank"));

				CComPtr<IAxWinAmbientDispatch> spHost = NULL;
				if (SUCCEEDED(axWindowContainer.QueryHost(IID_IAxWinAmbientDispatch, (LPVOID*)&spHost))) {
					//spHost->put_AllowContextMenu(VARIANT_FALSE);
					spHost->put_AllowShowUI(VARIANT_FALSE);
	...
	void PutStatusCtrlID(DWORD dwEditStatusID)
	{
	...
	BOOL HandleEvent(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		bHandled = FALSE;
		switch (uMsg)
		{
		case IE_WM_NOTIFY_CHANGE_EVENT:
		{
			...
			{
				SetDlgItemTextW(hDlg, dwEditStatusID, OLESTR("正在加载中..."));
			}
			break;
			case READYSTATE_LOADED:
			{
				SetDlgItemTextW(hDlg, dwEditStatusID, OLESTR("已加载。"));
			}
			break;
			case READYSTATE_INTERACTIVE:
			{
				SetDlgItemTextW(hDlg, dwEditStatusID, OLESTR("资源加载中..."));
			}
			break;
			case READYSTATE_COMPLETE:
			{
				SetDlgItemTextW(hDlg, dwEditStatusID, OLESTR("加载完成!"));
			}
			break;
			default:
			{
...
public:
	void EnumElementsOld(IHTMLDocument2* m_spDoc)
	{
		...
		for (int i = 0; i < len; i++)
		{
			...
			pCollec->item(varName, varIndex, &pDisp);
			if (NULL == pDisp)
			{
				continue;
			}
			...
			if (NULL != pElement)
			{
				...
				std::string strTag = StringConvertUtils::Instance()->WToA(bstrTag);
				std::cout << strTag.c_str() << std::endl;
			}
		}
	}
	void EnumIE(void)
